...
// Stage cross-fade (0..1) — multiplies the final alpha so the whole effect
// fades in/out with the scroll journey instead of popping.
...
// gl_FragColor = color;
  ...
  // Fade the whole effect via alpha so it cross-fades with adjacent stages.
  gl_FragColor = vec4(color.rgb * ray_intensity, uOpacity);
}